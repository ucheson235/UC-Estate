...
export default function SignUp() {
  ...
  async function onSubmit(e){
    ...
    try {
      ...
      toast.success("signup up was success")
      navigate("/");
         ...
         <div className='w-full md:w-[67%] lg:w-[40%] lg:ml-20'>
            <form  onSubmit={onSubmit}>
              ...
              <div className='relative mb-6'>
                ...
                />
                {showPassword ? (
                  <IoMdEyeOff className='absolute right-3 top-3 text-xl cursor-pointer' onClick={()=>setShowPassword((prevState) => ! prevState)} />
                ):(
                  <IoEye className='absolute right-3 top-3 text-xl cursor-pointer' onClick={()=>setShowPassword((prevState) => ! prevState)} />
                  )}
                </div>
                <div className='flex justify-between whitespace-nowrap text-sm sm:text-lg'>
                   <p className='mb-6'>
                      Have an account ?
                    <Link to="/Login" className='text-red-600 hover:text-red-700 transition duration-200 ease-in-out ml-1'> Login</Link>
                   </p>
