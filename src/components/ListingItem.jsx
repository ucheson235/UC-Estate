        ...
        transition-shadow duration-150 m-[10px]'>
         <Link className='contents' to={`/category/${listing.type}/${id}`}>
           <img className='h-[170px] w-full object-cover hover:scale-105 transition-scale
            ...
            />
           <Moment className="absolute top-2 left-2 bg-[#3377cc] text-white
           uppercase text-xs font-semibold rounded-md px-2 py-1 shadow-lg " fromNow>{listing.timestamp.toDate()}</Moment>
           <div className='w-full p-[10px]'>
             ...
             <p className='text-[#457b9d] mt-2 font-semibold'>${listing.offer ? listing.discountedPrice
              ...
              <div className='flex items-center mt-[10px] space-x-3'>
                <div className=' flex items-center space-x-1'>
                   <p className='font-bold text-xs'>{listing.bedroom > 1 ? `${listing.bedroom} beds ` : "1 Bed" }</p>
                </div>
                <div className='font-bold text-xs'> 
                  <p className='flex items-center space-x-1'>{listing.bathrooms > 1 ? `${listing.bedrooms} Baths` : "1 Bath"}</p>
